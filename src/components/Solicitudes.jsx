...
const Solicitudes = () => {
  ...
  const getData = async () => {
    try{
      const {data: response} = await axios.get('proyecto-integrador-back-production.up.railway.app/form')
      setData(response)
    } catch (error) {
  ...
  const confirmedDelete = () => {
    axios.delete(`proyecto-integrador-back-production.up.railway.app/delete-row/${idform}`).then(() =>{
    setOpen2(false)
    getData()
  ...
  const buttonUpdate = (() => {
    axios.put(`proyecto-integrador-back-production.up.railway.app/update-row/${idform}`, {idform, nombre, correo, telefono, solicitud, comentario})
    .then(()=>{
      getData()
      setNombre("")
      setCorreo("")
  ...
  const buttonAddUser = () =>{
    if (user === "" || password === ""){
     ...
     axios.post('proyecto-integrador-back-production.up.railway.app/add-user', {user, password})
     .then((response) => {
       setUser("")
       setPassword("")
       setShowSuccessAlert(true)
